...
I just changed the function checkForAdditionalData and added some comments
...
//enplus
  ...
  bool IsAnimating;//ajout
  bool CameraRotationInertiaIsEnabled;//ajout
  ...
  vesKiwiCameraSpinner::Ptr CameraSpinner;//ajout
...
{
    LOGI("vesKiwiViewerApp constructor");
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::initGL()//ajout
{
...
{
  return 6;
  //return this->Internal->BuiltinDatasetNames.size();
}
...
void vesKiwiViewerApp::addBuiltinDataset(const std::string& name, const std::string& filename)
{
  ...
  this->Internal->BuiltinDatasetCameraParameters.push_back(vesInternal::vesCameraParameters());//ajout
}

//----------------------------------------------------------------------------
void vesKiwiViewerApp::applyBuiltinDatasetCameraParameters(int index)//ajout
{
...
//----------------------------------------------------------------------------
bool vesKiwiViewerApp::isAnimating() const//ajout
{
  return this->Internal->IsAnimating;
}

//----------------------------------------------------------------------------
void vesKiwiViewerApp::setAnimating(bool animating)//ajout
{
...
void vesKiwiViewerApp::handleSingleTouchUp()
{
  ...
  if (!this->widgetInteractionIsActive()//ajout
    ...
    this->Internal->CameraSpinner->disable();//ajout
  }
...
void vesKiwiViewerApp::handleSingleTouchTap(int displayX, int displayY)
{
  ...
  this->Superclass::handleSingleTouchTap(displayX, displayY);//ajout
  for (size_t i = 0; i < this->Internal->DataRepresentations.size(); ++i) {
    vesKiwiWidgetRepresentation* rep = dynamic_cast<vesKiwiWidgetRepresentation*>(this->Internal->DataRepresentations[i]);
    if (rep) {
...
void vesKiwiViewerApp::handleSingleTouchDown(int displayX, int displayY)
{
  ...
  this->Superclass::handleSingleTouchDown(displayX, displayY);//ajout
  for (size_t i = 0; i < this->Internal->DataRepresentations.size(); ++i) {
    vesKiwiWidgetRepresentation* rep = dynamic_cast<vesKiwiWidgetRepresentation*>(this->Internal->DataRepresentations[i]);
    if (rep) {
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::handleTwoTouchPanGesture(double x0, double y0, double x1, double y1)//ajout
{
  this->Internal->CameraSpinner->disable();
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::handleTwoTouchPinchGesture(double scale)//ajout
{
  this->Internal->CameraSpinner->disable();
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::handleTwoTouchRotationGesture(double rotation)//ajout
{
  this->Internal->CameraSpinner->disable();
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::resetView()//ajout
{
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::resetScene()//ajout
{
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::setDefaultBackgroundColor()//different
{
  this->setBackgroundColor(63/255.0, 96/255.0, 144/255.0);
}

//----------------------------------------------------------------------------
void vesKiwiViewerApp::checkForAdditionalData(const std::string& path/*,const std::string& path*/)//different
{
    //LOGI("checkForAdditionalData(%s)-->not used", path);
    //vesNotUsed(path);


    /*FILE *f;
    f = fopen ("testErrors.txt", "w+");
    if (f!=NULL)
     {
         fprintf(f,"name %s, path %s",name.c_str(),path.c_str());
         fclose (f);
     }*/

    if (vtksys::SystemTools::FileExists(path.c_str()))
	{
      this->addBuiltinDataset(path, path);
    }
...
bool vesKiwiViewerApp::loadBrainAtlas(const std::string& filename)
{
  ...
  vesKiwiPlaneWidget* planeWidget = this->addPlaneWidget();//ajout
  rep->setClipPlane(planeWidget->plane());//ajout

  this->setBackgroundColor(0., 0., 0.);//ajout
  return true;
}
...
bool vesKiwiViewerApp::loadCanSimulation(const std::string& filename)
{
  ...
  rep->addSelfToRenderer(this->renderer());
  this->Internal->DataRepresentations.push_back(rep);
  this->setAnimating(true);//ajout
  return true;
}

//----------------------------------------------------------------------------
bool vesKiwiViewerApp::loadBlueMarble(const std::string& filename)//ajout
{
...
//----------------------------------------------------------------------------
bool vesKiwiViewerApp::loadDatasetWithCustomBehavior(const std::string& filename)//different
{
...
{
  if (this->Internal->ErrorMessage.empty()) {//ajout
    this->Internal->ErrorTitle = errorTitle;
    this->Internal->ErrorMessage = errorMessage;//ajout
  }
}

//----------------------------------------------------------------------------
void vesKiwiViewerApp::resetErrorMessage()//ajout
{
...
//----------------------------------------------------------------------------
void vesKiwiViewerApp::handleLoadDatasetError()//ajout
{
...
//----------------------------------------------------------------------------
std::string vesKiwiViewerApp::loadDatasetErrorTitle() const//different
{
  return this->Internal->ErrorTitle;
}

//----------------------------------------------------------------------------
std::string vesKiwiViewerApp::loadDatasetErrorMessage() const//different
{
  return this->Internal->ErrorMessage;
}

//----------------------------------------------------------------------------
void vesKiwiViewerApp::setCameraRotationInertiaIsEnabled(bool enabled)//ajout
{
  this->Internal->CameraRotationInertiaIsEnabled = enabled;
}

//----------------------------------------------------------------------------
bool vesKiwiViewerApp::cameraRotationInertiaIsEnabled() const//ajout
{
  return this->Internal->CameraRotationInertiaIsEnabled;
}

//----------------------------------------------------------------------------
void vesKiwiViewerApp::haltCameraRotationInertia()//ajout
{
  this->Internal->CameraSpinner->disable();
}

//----------------------------------------------------------------------------
vesKiwiCameraSpinner::Ptr vesKiwiViewerApp::cameraSpinner() const//ajout
{
