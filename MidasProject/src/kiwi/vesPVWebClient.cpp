...
bool vesPVWebClient::downloadObject(int objectIndex)
{
  ...
  url << "http://" << m_host << "/PWService/WebGL?"
      << "sid=" << m_sessionId
      << "&vid=" << m_viewId
      << "&q=" << "mesh"
      << "&id=" << id
...
jsonSharedPtr vesPVWebClient::rpc(const std::string& method, cJSON* params)
{
  ...
  std::stringstream resp;
  std::string url = "http://" + m_host + "/PWService/json";
...
bool vesPVWebClient::createVisualization()
{
  ...
  jsonSharedPtr result = this->rpc("VisualizationsManager.createVisualization", params);
...
jsonSharedPtr vesPVWebClient::executeCommand(const std::string& command, cJSON* params)
{
  ...
  return this->rpc("VisualizationsManager.invoke", invokeParams);
}
...
bool vesPVWebClient::createView()
{
  ...
  jsonSharedPtr result = this->executeCommand("CreateIfNeededRenderView");
